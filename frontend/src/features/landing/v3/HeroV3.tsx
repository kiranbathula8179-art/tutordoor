...
import { ArrowRight, BadgeCheck, CalendarCheck, Lock, Search, ShieldCheck, X } from "lucide-react";
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="container-page relative grid items-center gap-12 py-16 lg:grid-cols-[1.1fr_0.9fr] lg:py-24">
        <div>
          <motion.h1
            {...arriveProps(0.05)}
            className="max-w-xl font-display text-[2.6rem] font-extrabold leading-[1.06] tracking-[-0.02em] text-navy sm:text-5xl lg:text-[3.6rem]"
          >
            The right tutor for{" "}
          ...
          >
            ...
            <button
              type="submit"
              className="flex shrink-0 items-center gap-1.5 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-soft transition-all hover:bg-primary-dark hover:shadow-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
            >
              Search <ArrowRight className="h-4 w-4" />
            </button>
          </motion.form>

          <motion.div {...arriveProps(0.28)} className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2">
            {TRUST_CHIPS.map(({ icon: Icon, text }) => (
              <span key={text} className="flex items-center gap-1.5 text-sm font-medium text-slate-600">
        ...
        {/* The product as the illustration — real top-rated tutors */}
        {topTutors.length > 0 && (
